    ...
    public class PVPAtNight : RustPlugin
    {
        ...
        private void OnEntityDeath(BaseCombatEntity victimEntity, HitInfo hitInfo)
        {
            ...
            if (canPvp == true)
            {
                ...
                if (attacker is BasePlayer)
                {
                    ...
                    Server.Broadcast($"{attacker.ToPlayer().displayName} has received {item.Value}x {ItemManager.FindItemDefinition(item.Key).displayName} for killing {victimEntity.ToPlayer().displayName}");
                }
            }
        }
        ...
        void OnTimeSunset()
        {
            foreach (BasePlayer player in BasePlayer.activePlayerList)
            {
                ...
                Server.Command("gather.rate dispenser ore 3");
            }
        ...
        void OnTimeSunrise()
        {
            ...
            Server.Command("gather.rate dispenser ore 2");
            canPvp = false;
